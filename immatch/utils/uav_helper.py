'''
@File    :   uav_helper.py
@Time    :   2023/04/21 16:11:58
@Author  :   Xubo Luo 
@Version :   1.0
@Contact :   luoxubo@hotmail.com
@License :   (C)Copyright 2017-2018, Liugroup-NLPR-CASIA
@Desc    :   相对于hpatches_helper.py，本库对测试数据集的各任务的数目没有强制要求。
使用方法：将eval_hpatches.py中的'import immatch.utils.hpatches_helper as helper'改为
'import immatch.utils.uav_helper as helper'
'''
...
def eval_summary_matching(results, thres=[1, 3, 5, 10], save_npy=None, n_i=1, n_v=1):
    ...
    if n_i and n_v:
        summary += '#(Old)Matches: a={:.0f}, i={:.0f}, v={:.0f}\n'.format(
                        np.sum(n_matches) / ((n_i + n_v)), 
                        np.sum(n_matches[seq_type == 'i']) / (n_i), 
                        np.sum(n_matches[seq_type == 'v']) / (n_v)
                    )
        ...
        summary += '#(Old)Matches: a={:.0f}, i={:.0f}\n'.format(
                        np.sum(n_matches) / ((n_i + n_v)), 
                        np.sum(n_matches[seq_type == 'i']) / (n_i)
                    )
        summary += '#Matches: a={:.0f}, i={:.0f}\n'.format(
                        np.mean(n_matches),
                        np.mean(n_matches[seq_type == 'i'])
                    )
    elif n_v:
        summary += '#(Old)Matches: a={:.0f}, v={:.0f}\n'.format(
                        np.sum(n_matches) / ((n_i + n_v)), 
                        np.sum(n_matches[seq_type == 'v']) / (n_v)
                    )
        summary += '#Matches: a={:.0f}, v={:.0f}\n'.format(
                        np.mean(n_matches),
                        np.mean(n_matches[seq_type == 'v'])
                    )
    
    thres = np.array(thres)
    ierr = np.array([i_err[th] / (n_i) for th in thres ])
    verr = np.array([v_err[th] / (n_v) for th in thres])
    
    if n_i == 0:
        ierr = np.array([0.0 for th in thres ])
    if n_v == 0:
        verr = np.array([0.0 for th in thres])

    aerr = np.array([(i_err[th] + v_err[th]) / ((n_i + n_v)) for th in thres])
...
def eval_hpatches(
    ...
    # Matching
    if 'matching' in task:
        thres_range = np.arange(1, thres[-1]+1)
    ...
    for seq_idx, seq_dir in tqdm(enumerate(seq_dirs[::-1]), total=len(seq_dirs), smoothing=.5):
        ...
        im1_path = os.path.join(seq_dir, '1.JPG')

        # Eval on composed pairs within seq
        for im_idx in range(2, 3):
            im2_path = os.path.join(seq_dir, '{}.JPG'.format(im_idx))
            H_gt = np.loadtxt(os.path.join(seq_dir, 'H_1_{}.txt'.format(im_idx)))
            scale = np.ones(4)
    ...
    if 'matching' in task:
        ...
        lprint_(eval_summary_matching(results, thres, save_npy=save_npy, n_i = np.sum(seq_type=='i'), n_v = np.sum(seq_type=='v')))
    if 'homography' in task:
